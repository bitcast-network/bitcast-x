...
- A **campaign publisher** publishes the campaign manifest and immutable, digest-addressed
  ecosystem maps. These define campaign timing, creator eligibility, content requirements,
  attribution mode, and reward inputs.
...
windows, and incomplete emission bounds. A validator persists campaign routing and rejects a later
attempt to change an existing campaign between `legacy_connection` and `preclaim_v2`.
