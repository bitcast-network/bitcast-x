...
def test_campaign_protocol_cannot_change_during_transition(tmp_path) -> None:
    store = ValidatorStore(tmp_path / "validator.sqlite3")
    store.bind_campaign_protocols((campaign("same", MiningProtocol.LEGACY_CONNECTION),))

    with pytest.raises(ProtocolError, match="changed mining_protocol"):
        store.bind_campaign_protocols((campaign("same", MiningProtocol.PRECLAIM_V2),))


def test_campaign_exclusive_miner_cannot_change_after_observation(tmp_path) -> None:
    store = ValidatorStore(tmp_path / "validator.sqlite3")
    store.bind_campaign_protocols((campaign("same", MiningProtocol.PRECLAIM_V2, HOTKEY),))

    with pytest.raises(ProtocolError, match="changed exclusive_miner_hotkey"):
        store.bind_campaign_protocols(
            (campaign("same", MiningProtocol.PRECLAIM_V2, "5" + "F" * 47),)
        )
...
def test_complete_campaign_contract_cannot_change_after_first_observation(
    tmp_path, field: str
) -> None:
    store = ValidatorStore(tmp_path / "validator.sqlite3")
