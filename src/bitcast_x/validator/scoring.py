...
class AttributionScorer:
    ...
    async def score(
        ...
        *,
        ...
        """Score accepted results concurrently while preserving deterministic output order."""
        ...
        scored = [self._score_one(feed, item, *evidence[item.tweet_id]) for item in accepted]
        if self._brief_filter is not None:
            evaluations = await asyncio.gather(*(self._evaluate(feed, item) for item in scored))
            scored = [
                item.model_copy(
                    update={
