...
class CampaignReconciler:
    ...
    async def reconcile_campaign(
        ...
        *,
        ...
        """Replay one frozen campaign and return one auditable result per submitted tweet."""
        ...
        for tweet_id in ordered_tweets:
            candidates = sorted(by_tweet[tweet_id], key=lambda item: item.order)
            candidates = [
                item
            ...
            if not candidates:
                output.append(self._reject(tweet_id, campaign, AttributionReason.LATE_SUBMISSION))
                continue
            tweet = fetched[tweet_id]
            if tweet is None:
                output.append(self._reject(tweet_id, campaign, AttributionReason.TWEET_NOT_FOUND))
                continue
            campaign_failure = self._campaign_failure(campaign, feed, tweet)
            if campaign_failure is not None:
                output.append(self._reject(tweet_id, campaign, campaign_failure))
                continue
            if campaign.access.exclusive_miner_hotkey is not None:
                output.append(
                    await self._exclusive(campaign, tweet, candidates, through_block=through_block)
                )
                continue
